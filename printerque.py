class Queue():
    ...
    def add_jobs(self, docs):
        new_queue = new_queue.self.empty()
        if new_queue.self.is_empty() is True:
            new_queue.self.enque(docs)
            return new_queue
    ...
    def swap(self, item):
        ...
        # self._length -= 1 (I plan to delete this line)
        node = self._head
        self._head = self._head.push(item)

        if not self._head:
            self._tail = None

# add method that returns the total # of nodes that are present in the tree
# use a  stack or queue to keep track of the nodes visited and use a while loop that exits when the stack is empty
